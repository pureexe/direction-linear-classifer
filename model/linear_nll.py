...
class LinearClassifer(pl.LightningModule):
    def __init__(self, in_features, out_features):
        ...
        self.loss_fn = F.nll_loss #@see https://github.com/pytorch/examples/blob/main/mnist/main.py
    ...
    def configure_optimizers(self):
        ...
        lambda_fn = lambda epoch: 0.1 ** (epoch // 10)  if epoch < 20 else 0.1 ** 2
    ...
    def calcurate_accuracy(self, x_hat, y, label=""):
        ...
        for i in range(self.out_features):
            ...
            distance = torch.sum((x_label[inds] == i)) / total_inds
            acc = 1.0 - distance
    ...
    def calcurate_accuracy_old(self, x_hat, y, label=""):
        x_hathot = torch.zeros_like(x_hat)
        #x_hat_softmax = torch.nn.functional.softmax(x_hat, dim=1)
        x_hathot[:, torch.argmax(x_hat,dim=1)] = 1
        avg_acc = []
        for i in range(self.out_features):
            inds = y[:,i] == 1
            total_inds = y[inds].shape[0]
